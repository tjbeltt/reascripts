...
local WINDOW_TITLE = 'TJ Song switcher'
...
local SIGNALS = {
  ...
  repeatsong=function() repeatSongatIndex() end,
  togglestop=function() toggleStopSwitch() end,
}
...
function loadTracks()
  ...
  for index=0,reaper.GetNumTracks()-1 do
    ...
    elseif depth >= 1 and isSong then
      ...
      if not song.midiNote then --or song.midiNote < index then
        song.midiNote = MIDINOTE + 1
        MIDINOTE = MIDINOTE + 1
      end
...
function setSongEnabled(song, enabled)
  ...
  if not (reaper.GetTrackState(song.folder) == TRACK_NAME_TO_SKIP) then
...
function toggleStopSwitch() -- WORK IN PROGRESS
   reaper.SetExtState(EXT_SECTION, EXT_SWITCH_MODE, SWITCH_ALL, true)
end

function playMidiNoteForSong(kbmode, note, i, vel)
  reaper.StuffMIDIMessage(kbmode, note, songs[i].midiNote, vel)
end

function repeatSongatIndex()
  local virtualKeyboardMode = 0
  local channel = getCurrentNoteChannel()
  local noteOnCommand = 0x90 + channel
  local velocity = 96
...
function contextMenu()
  ...
  local menu = {
    ...
    separator,
    'Current Song: "' .. songs[currentIndex].name:sub(4) .. '"',
    'Midi Note:  ' .. songs[currentIndex].midiNote,
    'Midi Note Name:  ' .. getNoteName(songs[currentIndex].midiNote),
    separator,
  }
...
function donothing() -- utility function called in contextmenu() for line items that I don't want to do anything
end

function getNoteName(notenum) -- Thanks Fabian (& Suzuki) https://forum.cockos.com/showpost.php?p=2521073&postcount=5
